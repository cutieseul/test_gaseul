...
public class SelectDetail_dao {
	...
	public ProductDetail_dto all (String spCode) {
		...
		try {
			...
			String query = "select pCode, pImg_detail, pImg_main, pName, pQuantity, pPrice, pClick from product where pCode = ?";
			...
			if(resultSet.next()) {
				...
				dto = new ProductDetail_dto (pCode, pImg_detail, pImg_main, pName, pQuantity, pPrice, pClick);
			}
